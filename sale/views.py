...
def post_ad(request, category):
    ...
    if edit_token:
        try:
            ad = SaleAd.objects.get(edit_token=edit_token)
            ...
            context['edit'] = True
            response = render(request, 'sale/writead.html', context)
            if addcookie:
    ...
    if request.method == 'POST':
        ...
        if delete == 'true':
            try:
                ad = SaleAd.objects.get(id=request.POST['adid'])
        ...
        if request.POST['nextstep'] == '1':
            ...
            context['S3_IMG_BUCKET'] = img_bucket
            context['bad_words_list'] = settings.FILTER_WORDS_HEX
            return render(request, 'sale/writead.html', context)
        ...
        if request.POST['nextstep'] == '3':
            ...
            return render(request, 'sale/donead.html', context)
        ...
        if adid:
            ad = SaleAd.objects.get(id=adid)
        else:
            ad = SaleAd(status='pending') #pending
            ad.category = cat
            ad.city = city
        ...
        return render(request, 'sale/previewad.html', context)
...
def listings(request, category_slug):
    ...
    response = render(request, 'sale/listings.html', context)
    if addcookie:
        return utils.render_with_cookie(response, city)
    else:
        return response
...
def ad_detail(request, category_slug, ad_id):
    ...
    try:
        ...
        if subdomain:
            city = City.objects.get(short_name=subdomain)
            return redirect('{}/sale/{}'.format(city.get_city_url(), category_slug))
        else:
            return redirect(site_url)
    ...
    if ad.status != 'live':
        if ad.creator:
            if not ad.creator.pk == request.user.pk:
                return redirect('%s/sale/%s' % (city.get_city_url(), category_slug))
        else:
            return redirect('%s/sale/%s' % (city.get_city_url(), category_slug))
    ...
    context['categories'] = categories
    context['category'] = category
    response = render(request, 'sale/ad_detail.html', context)
    if addcookie:
        return utils.render_with_cookie(response, city)
    else:
        return response
